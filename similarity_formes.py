...
def load_vector(json_path):
    with open(json_path, "r") as f:
        data = json.load(f)
    return np.array(data["fourier"] + data["direction_hist"], dtype=float)

def distance(v1, v2):
    return np.linalg.norm(v1 - v2)

def show_image(title, img_path, distance=None):
        # Cette fonction n'est plus utilisée pour l'affichage individuel
        pass
...
def main():
    ...
    # Charger vecteur de l’image requête
    ...
    # Préparer les images à afficher dans la grille
    top_images = [(query_img.replace('.gif',''), 0.0, query_img_cv)]
    for name, d in distances[:6]:
        print(f"  ➤ {name}   (distance = {d:.4f})")
        img_path = os.path.join(FOLDER, name + ".gif")
